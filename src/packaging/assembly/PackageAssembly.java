...
public class PackageAssembly implements IPackageAssembly {
    ...
    private Package assemblePackage(TypeOrder order) {
        ...
        while(order.getParts().size() < order.quantity){
            for (int i = order.getParts().size(); i < order.quantity; i++) {
                try {
                    CarPart carPart = Util.convertToModel(DB.readCarPartByType(order.getPartType()));
                    if (carPart == null) {
                        if (isLookingForMissingCarParts) {
                            System.out.println("There is no Car Part of a type: " + order.getPartType() + " . Waiting...");
                            try {
                                Thread.sleep(3000);
                    ...
                    DB.updateCarPartReferenceToPackage(carPart.getId(),order.getPackageNo());
                } catch (RemoteException e) {
                    e.printStackTrace();
                }
            }
    ...
    private Package assemblePackage(PresetOrder order) {
        ...
        while(order.getParts().size() < order.preset.partTypes.length){
            for (int i = order.getParts().size(); i < order.preset.partTypes.length; i++) {
                try {
                    if(order.preset.partTypes[i] == null)
                        continue;
                    CarPart carPart = Util.convertToModel(DB.readCarPartByTypeAndModel(order.preset.partTypes[i], order.getCarModel()));
                    if(carPart == null){
                        if(isLookingForMissingCarParts){
                            System.out.println("There is no Car Part of a type: " + order.preset.partTypes[i] + " . Waiting...");
                            try {
                                Thread.sleep(3000);
                            } catch (InterruptedException e) {
                                e.printStackTrace();
